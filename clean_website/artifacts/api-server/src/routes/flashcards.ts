...
router.post("/materials/:id/flashcard-decks", generationRateLimiter, async (req, res) => {
  ...
  // Vocab materials use the deterministic path: every word becomes one card,
  // re-shuffled on every call — no AI, no cost. All other subject types go
  ...
  const cards = isVocab && vocabEntries.length > 0
