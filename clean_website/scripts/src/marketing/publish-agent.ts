...
// Loads secrets (META_ACCESS_TOKEN, etc.) from clean_website/.env when
...
// Every video that reaches this queue came from a video-agent-rendered idea
// (see video-agent.ts), which only ever tags ideas channel_hint:"video" --
// so there's no per-idea signal for "Instagram vs Facebook", and a finished
...
// container from the video URL (a time-limited signed GCS URL minted by
// video-agent.ts, valid long enough for Meta to fetch it during this flow).
...
// Uses the file_url form (the video is hosted at a signed, time-limited GCS
// URL minted by video-agent.ts) rather than the resumable chunked-upload
// flow -- simpler, but less exercised in Meta's current docs than the
// Instagram Reels flow above. If a Page/App combination rejects file_url,
// Graph API's error response is surfaced verbatim in facebook_publish.error
// for diagnosis.
